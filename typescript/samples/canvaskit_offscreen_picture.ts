...
let pict = recorder.finishRecordingAsPicture();
let buffer = std.Buffer.MakeFromAdoptBuffer(pict.serialize());

let file = await std.File.Open('./output.skp', std.File.O_RDWR | std.File.O_CREAT, 0o644);
