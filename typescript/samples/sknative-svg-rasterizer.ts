...
function main(): void {
    ...
    const surface = GL.CkSurface.MakeRaster(GL.CkImageInfo.MakeSRGB(
        width, height, GL.Constants.COLOR_TYPE_BGRA8888, GL.Constants.ALPHA_TYPE_OPAQUE));
    ...
    std.File.WriteFileSync(output, std.Buffer.MakeFromAdoptBuffer(new Uint8Array(encoded)));
}
