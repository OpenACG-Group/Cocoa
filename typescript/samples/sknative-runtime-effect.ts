...
const win = await Window.Create(display, 800, 600, true);
win.eventHandler = {
    onFrame() {
        render();
    },
...
// A fancy simulation of blue sky with running clouds
// From: https://shaders.skia.org/?id=f9be5248170044ea1b69ee78456eec4be98d3e71a6c61fd0138f6018abda2ac3
const SOURCE = `
uniform float iTime;
...
const effect = GL.CkRuntimeEffect.MakeForShader(SOURCE, false, (error) => {
...
function render(): void {
    ...
    const currentTimeMs = getMillisecondTimeCounter();

    const iTime = (currentTimeMs - startTimeMs) / 1000;

    const paint = new GL.CkPaint();
    paint.setShader(effect.makeShader([ iTime, 800, 600 ], [], null));
