...
type_name_cpp_convert_dict = {
    ...
    # Memory operations
    'u32array': 'IRHeap::U32Array',
    'f32array': 'IRHeap::F32Array',
    'vec2': 'SkV2',
    'vec3': 'SkV3',
    'vec4': 'SkV4',
    'mat4x4': 'SkM44',
...
def codegen_typescript():
    print('import { IProtoBufferWriter } from \'../cocanvas_iface\';')
...
def codegen_cpp_header():
    ...
    for entry in enum_map_entries:
        ...
        print(f"inline {cpp_type_name} vrir_enum_translator_{entry['type_name']}({underlying_type} x) {{")
        print(f'  static {cpp_type_name} __tbl[] = {{')
        for member in entry['members']:
            print(f"    {member['cpp_value']},")
        print('  };')
        print(f"  if (x < 0 || x >= {len(entry['members'])})")
        print(f"    g_throw(Error, \"Invalid enumeration value for type {entry['type_name']}\");")
    ...
    for entry in codegen_entries:
        ...
        for operand in params:
            ...
            if operand['memop'] is True and typename != 'any':
                executor_arg += '& '
            executor_arg += operand['value']
            cpp_executor_args += ', ' + executor_arg

        if entry['impl'] is None:
            print(f'void vrir_executor_{name}(IRHeap& heap{cpp_executor_args});')
        else:
            print(f"void vrir_executor_{name}(IRHeap& heap{cpp_executor_args}) {{\n{entry['impl']}\n}}")
...
struct OpcodeVectorTbl {
  ...
  struct {
    ...
    for entry in codegen_entries:
        ...
        print(f'  {{ k{opcode_enum_name},  "{name}", {{ {operands_field} }} }},')
    print('}; // g_opcode_vector_tbl')

    # Generate instruction dispatcher
    print('void Dispatch(IRStreamReader& reader, IRHeap& heap, SkCanvas *canvas) {')
    print('''
  while (true) {
        ...
        for operand in entry['params']:
            ...
            if operand['memop'] is True:
                ...
                if maybe_enum_map_entry is None:
                    ...
                    print(f"        auto x{param_count} = vrir_enum_translator_{enum_name}(reader.extractNext<{underlying_type}>());")
            param_count += 1

        executor_call_args = 'heap'
        for index in range(param_count):
            executor_call_args += f', x{index}'

        print(f'        vrir_executor_{name}({executor_call_args});')
        print('        break;\n      }')
...
if len(sys.argv) != 3:
    ...
    print('Available targets: cpp, typescript')
    exit(1)
