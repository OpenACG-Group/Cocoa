...
#include "Cobalt/Wayland/WaylandHWComposeRenderTarget.h"
#include "Cobalt/Wayland/WaylandDisplay.h"
#include "Cobalt/HWComposeSwapchain.h"
COBALT_NAMESPACE_BEGIN

#define THIS_FILE_MODULE COCOA_MODULE_NAME(Cobalt.Wayland.HWComposeRenderTarget)

namespace {

struct WaylandVkSurfaceFactory : public HWComposeContext::VkSurfaceFactory
{
    explicit WaylandVkSurfaceFactory(co_sp<WaylandHWComposeRenderTarget> rt)
        : hw_compose_rt_(std::move(rt)) {}
    ~WaylandVkSurfaceFactory() = default;

    VkSurfaceKHR Create(const co_sp<HWComposeContext> &context) override
    {
    ...
    co_sp<WaylandHWComposeRenderTarget> hw_compose_rt_;
};

} // namespace anonymous

co_sp<WaylandHWComposeRenderTarget>
WaylandHWComposeRenderTarget::Make(const co_sp<WaylandDisplay>& display,
...
WaylandHWComposeRenderTarget::WaylandHWComposeRenderTarget(co_sp<HWComposeContext> hwContext,
