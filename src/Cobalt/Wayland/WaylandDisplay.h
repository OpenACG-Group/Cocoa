#ifndef COCOA_COBALT_WAYLAND_WAYLANDDISPLAY_H
#define COCOA_COBALT_WAYLAND_WAYLANDDISPLAY_H
...
#include "Cobalt/Wayland/protos/xdg-decoration-unstable-protocol.h"
#include "Cobalt/Wayland/protos/kde-server-decoration-protocol.h"
...
public:
    ...
    g_nodiscard g_inline const co_unique<Globals>& GetGlobalsRef() {
        return globals_;
    }
...
private:
    co_sp<Surface> OnCreateSurface(int32_t width, int32_t height, SkColorType format,
...
{
public:
    explicit WaylandRoundtripScope(co_sp<WaylandDisplay> display);
    ~WaylandRoundtripScope();

private:
    co_sp<WaylandDisplay>   display_;
    bool                    changed_;
};

COBALT_NAMESPACE_END
#endif //COCOA_COBALT_WAYLAND_WAYLANDDISPLAY_H
