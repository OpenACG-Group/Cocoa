...
#define THIS_FILE_MODULE COCOA_MODULE_NAME(Cobalt.Wayland.Surface)
...
const xdg_toplevel_listener g_xdg_toplevel_listener = {
    toplevel_configure_callback,
    toplevel_close_callback
};

}

co_sp<Surface> WaylandSurface::Make(const co_sp<WaylandRenderTarget>& rt)
{
...
WaylandSurface::WaylandSurface(const co_sp<WaylandRenderTarget>& rt)
    : Surface(rt)
    , wl_display_(nullptr)
