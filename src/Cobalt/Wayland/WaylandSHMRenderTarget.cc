...
#include "Cobalt/Wayland/WaylandUtils.h"
#include "Cobalt/Wayland/WaylandDisplay.h"
#include "Cobalt/Wayland/WaylandSHMRenderTarget.h"
COBALT_NAMESPACE_BEGIN

#define THIS_FILE_MODULE COCOA_MODULE_NAME(Cobalt.Wayland.SHMRenderTarget)

#define RT_INITIAL_BUFFERS  3
#define RT_EMPTY_INDEX      (-1)

co_sp<WaylandSHMRenderTarget>
WaylandSHMRenderTarget::Make(const co_sp<WaylandDisplay>& display,
...
WaylandSHMRenderTarget::WaylandSHMRenderTarget(const co_sp<WaylandDisplay>& display,
...
int create_shared_memory_fd()
{
    ...
    do
    {
        ...
        if (!forceShmOpen)
        {
            ...
            ret = memfd_create("Cocoa.Cobalt.Wayland.RenderTarget", options);
        ...
        if (forceShmOpen)
        {
            auto name = fmt::format("/Cocoa.Cobalt.Wayland.RenderTarget#{:x}", random());
...
void WaylandSHMRenderTarget::BufferReleaseCallback(void *data, g_maybe_unused wl_buffer *buffer)
{
    ...
    if (self->state == WaylandSHMRenderTarget::BufferState::kDeferredDestroying)
    {
        ...
        auto itr = std::find_if(self->rt->deferred_destructing_buffers_.begin(),
                                ...
                                [self](const co_unique<Buffer>& v) -> bool {
            return (v.get() == self);
        });
...
{
    for (co_unique<Buffer>& ptr : buffers_)
    {
...
SkSurface *WaylandSHMRenderTarget::OnBeginFrame()
{
    ...
    co_unique<Buffer>& buf = buffers_[drawing_buffer_idx_];
...
void WaylandSHMRenderTarget::OnSubmitFrame(SkSurface *surface, const SkRegion& damage)
{
    ...
    co_unique<Buffer>& committed = buffers_[committed_buffer_idx_];
...
std::string WaylandSHMRenderTarget::GetBufferStateDescriptor()
{
    ...
    std::string out;
    int32_t idx = 0;
    for (const co_unique<Buffer>& buffer : buffers_)
    {
