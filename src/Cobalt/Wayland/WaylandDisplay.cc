...
#include "Cobalt/Wayland/WaylandSurface.h"
...
void WaylandDisplay::PollCallback(uv_poll_t *poll, int status, int events)
{
    ...
    else if (events == UV_READABLE)
    {
        ...
        wl_display_dispatch_pending(d->wl_display_);
        for (const co_sp<Surface>& surface : d->GetSurfacesList())
        {
...
WaylandRoundtripScope::WaylandRoundtripScope(co_sp<WaylandDisplay> display)
...
co_sp<Surface> WaylandDisplay::OnCreateSurface(int32_t width, int32_t height, SkColorType format,
                                               RenderTarget::RenderDevice device)
{
    WaylandRoundtripScope scope(Self()->Cast<WaylandDisplay>());

    co_sp<WaylandRenderTarget> rt;
    switch (device)
    {
