...
#define THIS_FILE_MODULE COCOA_MODULE_NAME(Cobalt.HWComposeSwapchain)
...
GpuQueueFamilyIndices find_gpu_queue_families(const co_sp<HWComposeContext>& ctx, VkSurfaceKHR surface)
{
...
VkDevice create_gpu_logical_device(const co_sp<HWComposeContext>& ctx, VkSurfaceKHR surface,
...
void populate_gr_vk_extensions(GrVkExtensions& ext, const co_sp<HWComposeContext>& ctx)
{
...
co_sp<HWComposeSwapchain> HWComposeSwapchain::Make(const co_sp<HWComposeContext>& context,
...
HWComposeSwapchain::HWComposeSwapchain(co_sp<HWComposeContext> ctx, VkSurfaceKHR surface)
