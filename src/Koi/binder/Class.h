#ifndef COCOA_KOI_BINDER_CLASS_H
#define COCOA_KOI_BINDER_CLASS_H
...
struct ClassInfo
{
    ...
    koi_nodiscard std::string class_name() const;
};
...
public:
    ...
    koi_nodiscard bool auto_wrap_objects() const
...
public:
    ...
    set(std::string_view name, Function&& func, v8::PropertyAttribute attr = v8::None)
    {
        v8::Local<v8::Data> wrapped_fun = wrap_function_template(isolate(), std::forward<Func>(func));
        class_info_.class_function_template()
                ->PrototypeTemplate()->Set(to_v8(isolate(), name), wrapped_fun, attr);
        class_info_.js_function_template()->Set(to_v8(isolate(), name), wrapped_fun, attr);
        return *this;
    }
...
KOI_BINDER_NS_END
#endif //COCOA_KOI_BINDER_CLASS_H
