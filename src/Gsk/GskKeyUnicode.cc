...
uint32_t KeyvalToUnicode(uint32_t keyval)
{
    ...
    int min = 0;
    int max = sizeof(keysym_to_unicode_tab_) / sizeof(keysym_to_unicode_tab_[0]) - 1;
    int mid;
    /* binary search in table */
    while (max >= min) {
        mid = (min + max) / 2;
        if (keysym_to_unicode_tab_[mid].keysym < keyval)
            min = mid + 1;
        else if (keysym_to_unicode_tab_[mid].keysym > keyval)
            max = mid - 1;
        else {
            /* found it */
            return keysym_to_unicode_tab_[mid].ucs;
        }
    }
...
uint32_t UnicodeToKeyval(uint32_t wc)
{
    ...
    int min = 0;
    int max = sizeof(unicode_to_keysym_tab_) / sizeof(unicode_to_keysym_tab_[0]) - 1;
    int mid;

    /* Binary search in table */
    while (max >= min) {
        mid = (min + max) / 2;
        if (unicode_to_keysym_tab_[mid].ucs < wc)
            min = mid + 1;
        else if (unicode_to_keysym_tab_[mid].ucs > wc)
            max = mid - 1;
        else {
            /* found it */
            return unicode_to_keysym_tab_[mid].keysym;
        }
    }
