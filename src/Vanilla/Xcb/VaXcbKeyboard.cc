...
VaXcbKeyboard::VaXcbKeyboard(VaXcbDisplay *display)
    : fDisplay(display),
      fXkbContext(nullptr),
      fXkbKeymap(nullptr),
      fXkbState(nullptr),
      ...
      fCoreDeviceId(0)
{
    ...
    xkb_x11_setup_xkb_extension(connection,
                                ...
                                &major, &minor,
                                nullptr, nullptr);
    Journal::Ref()(LOG_INFO, "<input> XKB extension for X11 has been detected, version {}.{}",
                   major, minor);
    ...
    Journal::Ref()(LOG_INFO, "<input> Using XInput keyboard device #{}", deviceId);
    ...
    fProxy = std::make_unique<VaKeyboardProxy>(fXkbContext, fXkbKeymap, fXkbState);
}

VaXcbKeyboard::~VaXcbKeyboard()
{
...
void VaXcbKeyboard::handleXkbEvent(const xcb_generic_event_t *event)
{
    ...
    case XCB_XKB_MAP_NOTIFY:
        Journal::Ref()(LOG_DEBUG, "XKB map notify");
        break;

    case XCB_XKB_NEW_KEYBOARD_NOTIFY:
        Journal::Ref()(LOG_DEBUG, "XKB new keyboard notify");
        break;
    }
}

KeySymbol VaXcbKeyboard::symbol(xcb_keycode_t code)
{
