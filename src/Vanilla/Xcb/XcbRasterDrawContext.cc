...
XcbRasterDrawContext::XcbRasterDrawContext(Handle<Window> window)
    : DrawContext(std::move(window), RasterizerType::kRaster),
      fConnection(nullptr),
      fWindow(0),
      fXcbGc(0),
      ...
      fDepth(0)
{
    auto xcbWindow = std::dynamic_pointer_cast<XcbWindow>(getWindow());
    auto xcbDisplay = std::dynamic_pointer_cast<XcbDisplay>(getWindow()->getDisplay());
...
void XcbRasterDrawContext::createSurface(int32_t width, int32_t height)
{
    if (fSurface)
        fSurface = nullptr;
    if (fPixelMemory)
...
XcbRasterDrawContext::~XcbRasterDrawContext()
{
    if (fSurface)
        fSurface = nullptr;
    if (fPixelMemory)
...
void XcbRasterDrawContext::onResize(int32_t width, int32_t height)
{
    createSurface(width, height);
}

sk_sp<SkSurface> XcbRasterDrawContext::onBeginFrame(const SkRect& region)
{
    return fSurface;
}

void XcbRasterDrawContext::onEndFrame(const SkRect& region)
{
