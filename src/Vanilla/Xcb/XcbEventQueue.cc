...
XcbEventQueue::XcbEventQueue(XcbDisplay *display)
    : AsyncSource(display->getContext()->eventLoop()),
      fDisplay(display),
      fMainThreadId(std::this_thread::get_id()),
      fDisposed(false),
      fThread(&XcbEventQueue::entrance, this)
{
}
...
void XcbEventQueue::asyncDispatch()
{
    fQueueMutex.lock();
...
void XcbEventQueue::disposeFromMainThread()
{
    if (fDisposed)
        return;
    ...
    event.type = fDisplay->atom(XcbAtoms::_VA_CLOSE_CONNECTION);
...
bool XcbEventQueue::isDisposeEvent(xcb_generic_event_t *pEvent)
{
    if (pEvent && (pEvent->response_type & ~0x80) == XCB_CLIENT_MESSAGE)
    {
        ...
        if (msg->type == fDisplay->atom(XcbAtoms::_VA_CLOSE_CONNECTION))
            return true;
    }
    return false;
}

bool XcbEventQueue::tryEnqueue(xcb_generic_event_t *event)
{
