...
#include "Vanilla/VaVkDrawContext.h"
#include "Vanilla/Xcb/VaXcbDisplay.h"
#include "Vanilla/Xcb/VaXcbWindow.h"
VANILLA_NS_BEGIN

Handle<VaDrawContext> VaDrawContext::MakeVulkan(Handle<VaWindow> window, bool enableDebug)
{
    VaVkDrawContext::SurfaceCreatorPfn surfaceCreator;
    ...
    case DisplayBackend::kDisplay_Xcb:
        ...
        surfaceCreator = [window](VkInstance instance) -> std::tuple<VkResult, VkSurfaceKHR> {
            auto x11Window = std::dynamic_pointer_cast<VaXcbWindow>(window);
            auto x11Display = std::dynamic_pointer_cast<VaXcbDisplay>(window->getDisplay());
    ...
    auto context = std::make_shared<VaVkDrawContext>(std::move(window), enableDebug,
...
bool vk_has_required_layer()
{
    ...
    for (const char *layerName : vk_required_layers)
    {
        ...
        if (!found)
        {
            Journal::Ref()(LOG_WARNING, "Vulkan layer {} is required but not available", layerName);
            return false;
        }
        Journal::Ref()(LOG_INFO, "Found Vulkan layer {}", layerName);
    }
    return true;
}
...
bool vk_has_required_instance_extensions(const std::vector<const char*>& require)
{
    ...
    Journal::Ref()(LOG_INFO, "Vulkan instance has {} available extension(s)", count);
    ...
    for (const auto& req : require)
    {
        ...
        if (!found)
        {
            Journal::Ref()(LOG_ERROR, "Vulkan extension {} not available", req);
            return false;
        }
    }
    return true;
                        ...
                        [[maybe_unused]] void *pUserData)
{
    ...
    Journal::Ref()(LOG_DEBUG, "%fg<hl>Vulkan%reset-{}: {}", psSeverity, pData->pMessage);
    return VK_FALSE;
}
...
VaVkDrawContext::VaVkDrawContext(Handle<VaWindow> window, bool enableDebug,
                                 std::vector<const char*> instanceExt,
                                 std::vector<const char*> deviceExt)
    : VaDrawContext(std::move(window), RasterizerType::kVulkan),
...
VaVkDrawContext::~VaVkDrawContext()
{
...
void VaVkDrawContext::initialize(const SurfaceCreatorPfn& surfaceCreator)
{
    uint32_t apiVersion;
    vkEnumerateInstanceVersion(&apiVersion);
    Journal::Ref()(LOG_INFO, "Vulkan Instance Version {}.{}.{}",
...
void VaVkDrawContext::createGpuInstance()
{
    /* Check the supporting of validation layer */
    if (fEnableDebug && !vk_has_required_layer())
    {
        Journal::Ref()(LOG_ERROR, "Vulkan debug is unavailable as validation layer can't be found");
        fEnableDebug = false;
    ...
    {
        Journal::Ref()(LOG_ERROR, "Failed to create Vulkan instance: {}", vk_strerror(result));
    ...
    if (fEnableDebug)
    {
        ...
        if (result != VK_SUCCESS)
        {
            Journal::Ref()(LOG_ERROR, "Failed to create Vulkan messenger: {}", vk_strerror(result));
                                    ...
                                    uint32_t& presentQueue)
{
    ...
    if (count == 0)
    {
        Journal::Ref()(LOG_ERROR, "None of physical devices are available");
    ...
    if (result != VK_SUCCESS)
    {
        Journal::Ref()(LOG_ERROR, "Failed to enumerate physical devices: {}", vk_strerror(result));
        return VK_NULL_HANDLE;
    }

    Journal::Ref()(LOG_INFO, "Found available physical devices:");
    int32_t chosen = -1;
    for (int32_t i = 0; i < count; i++)
    {
        VkPhysicalDeviceProperties properties;
        vkGetPhysicalDeviceProperties(phys[i], &properties);
        Journal::Ref()(LOG_INFO, "  [{}] {} [API Version {}.{}.{}]", i, properties.deviceName,
    ...
    if (chosen >= 0)
    {
        Journal::Ref()(LOG_INFO, "  Device #{} is chosen", chosen);
        return phys[chosen];
    }
    return VK_NULL_HANDLE;
}
...
void VaVkDrawContext::createGpuDevice(const SurfaceCreatorPfn& surfaceCreator)
{
    auto creatorResult = surfaceCreator(fInstance);
    if (std::get<0>(creatorResult) != VK_SUCCESS)
    {
        Journal::Ref()(LOG_ERROR, "Failed to create Vulkan surface: {}",
    ...
    if (result != VK_SUCCESS)
    {
        Journal::Ref()(LOG_ERROR, "Failed to create a Vulkan logical device: {}",
                vk_strerror(result));
...
void VaVkDrawContext::createDirectContext()
{
...
void VaVkDrawContext::checkSwapChain(int32_t width, int32_t height)
{
    if (width <= 0 || height <= 0)
    ...
    {
        Journal::Ref()(LOG_ERROR, "Failed to match a proper image format for Vulkan swapchian");
    ...
    if (result != VK_SUCCESS)
    {
        Journal::Ref()(LOG_ERROR, "Failed to create swapchain: {}", vk_strerror(result));
...
void VaVkDrawContext::createRenderTargets()
{
    ...
    for (int32_t i = 0; i < imageCount + 1; i++)
    {
        ...
        if (result != VK_SUCCESS)
        {
            Journal::Ref()(LOG_ERROR, "Failed to create Vulkan semaphore: {}", vk_strerror(result));
...
void VaVkDrawContext::destroyWholeSwapChain()
{
    fRTSurfaces.clear();
...
void VaVkDrawContext::onResize(int32_t width, int32_t height)
{
    if (fSwapChain != VK_NULL_HANDLE)
    {
        vkDeviceWaitIdle(fDevice);
...
sk_sp<SkSurface> VaVkDrawContext::onBeginFrame(const SkRect& region)
{
    ...
    if (!rt->acquired)
    {
        ...
        if (result != VK_SUCCESS)
        {
            ...
            if (result == VK_ERROR_OUT_OF_DATE_KHR)
            {
                 ...
                 * But the VaWindow object should notify us to do this later, so we just
                 * return nullptr to skip this frame. */
                return nullptr;
            }
            Journal::Ref()(LOG_ERROR, "Failed to acquire next Vulkan frame: {}",
                           vk_strerror(result));
...
void VaVkDrawContext::onEndFrame(const SkRect& region)
{
    Handle<VaVkRenderTarget> rt = fRenderTargets[fCurrentRT];
    if (!rt->acquired)
        return;
    ...
    {
        Journal::Ref()(LOG_ERROR, "Failed in submitting commands to GPU");
        return;
    }
