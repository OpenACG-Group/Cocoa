...
#include "Vanilla/Audio/PaConnection.h"
VANILLA_NS_BEGIN

#define THIS_FILE_MODULE COCOA_MODULE_NAME(Vanilla.Audio)

struct PaConnection::PrivateField
{
    PrivateField()
...
{
    auto pd = reinterpret_cast<PaConnection::PrivateField*>(userdata);
...
Handle<PaConnection> PaConnection::Make()
{
    ...
    // TODO: Volume control
    ::pa_threaded_mainloop_unlock(pd->mainloop);
    return std::make_shared<PaConnection>(std::move(pd));
}

PaConnection::PaConnection(UniqueHandle<PrivateField>&& data)
    : fData(std::move(data))
{
}

PaConnection::~PaConnection()
{
    this->close();
}
...
void wait_for_operation(PaConnection::PrivateField *pd, ::pa_operation *op)
{
    if (!op)
        return;
...
bool PaConnection::open(const AuSampleSpec& spec, uint32_t latency)
{
...
bool PaConnection::isOpen() const
{
    return (fData->stream != nullptr);
}

uint32_t PaConnection::play(const uint8_t *data, size_t size)
{
