                                      ...
                                      const std::vector<SignalEventInfo>& info_vec)
{
    for (const SignalEventInfo& event_info : info_vec)
    {
        ...
        this_->EmitterDefineEvent(event_info.name, [sp_event_data] {
            ...
            return sp_event_data->handle->Connect(sp_event_data->signum,

                [emit, sp_event_data](gl::PresentSignalArgs& info)
                {
                    TRACE_EVENT("present.signal", nullptr, [&](perfetto::EventContext& ctx)
                    {
                        auto type_name = gl::PresentRemoteHandle::GetTypeName(
                                sp_event_data->handle->GetRealType());
                        ctx.event()->set_name(fmt::format(
                                "signal:{}.{}", type_name, sp_event_data->name));
                    });
...
void PromisifiedRemoteCall::ResultCallback(gl::PresentRemoteCallReturn& info)
{
    ...
    {
        CHECK(resolver->Reject(context,
            v8::String::NewFromUtf8(isolate, error.c_str()).ToLocalChecked()).ToChecked());
        return;
    }
