...
#ifndef COCOA_GALLIUM_BINDINGS_PRESENT_PROMISIFY_H
#define COCOA_GALLIUM_BINDINGS_PRESENT_PROMISIFY_H
                                    ...
                                    size_t index)
{
    if constexpr (T::extract_value)
    {
        ...
        return ffi::Cast<typename T::cast_type>::ToChecked(isolate,
            static_cast<typename T::cast_type>(info.Get<typename T::real_type>(index).value()));
    }
    else if constexpr (T::create_obj)
    {
        return ffi::JSObject::New<typename T::cast_type>(isolate, info.Get<typename T::real_type>(index));
    }
    else if constexpr (std::is_same<typename T::real_type, typename T::cast_type>::value)
    {
        return ffi::Cast<typename T::real_type>::ToChecked(isolate, info.Get<typename T::real_type>(index));
    }
    else
    {
        return ffi::Cast<typename T::cast_type>::ToChecked(isolate,
...
SignalArgsVector GenericSignalArgsConverter(v8::Isolate *isolate, gl::PresentSignalArgs& info)
{
    size_t i = 0;
    SignalArgsVector result{
        // FIXME(present:sora): indexing by `i++` depends on the evaluation order,
        //                      which is an undefined behaviour in cpp.
                                      ...
                                      gl::PresentRemoteCallReturn& info)
{
    if constexpr (T::extract_value)
    {
        ...
        static_assert(!T::create_obj, "Illegal usage of SignalArgsCast<...>");
        return ffi::Cast<typename T::cast_type>::ToChecked(isolate,
            static_cast<typename T::cast_type>(info.GetReturnValue<typename T::real_type>().value()));
    }
    else if constexpr (T::create_obj)
    {
        return ffi::JSObject::New<typename T::cast_type>(isolate,
            info.GetReturnValue<typename T::real_type>());
    }
    else if constexpr (std::is_same<typename T::real_type, typename T::cast_type>::value)
    {
        return ffi::Cast<typename T::real_type>::ToChecked(isolate,
            info.GetReturnValue<typename T::real_type>());
    }
    else
    {
        return ffi::Cast<typename T::cast_type>::ToChecked(isolate,
            static_cast<typename T::cast_type>(info.GetReturnValue<typename T::real_type>()));
    }
}
                              ...
                              ReturnConverterF<Ret> return_converter)
{
    ...
    thread->SubmitTask<Ret>(std::move(task_func), [closure, return_converter](Ret value) {
        ...
        r->Resolve(closure->isolate->GetCurrentContext(), h).Check();
    }, [closure](std::string error) {
        v8::HandleScope scope(closure->isolate);
        auto r = closure->resolver.Get(closure->isolate);
        r->Reject(
            closure->isolate->GetCurrentContext(),
            v8::String::NewFromUtf8(closure->isolate, error.c_str()).ToLocalChecked()
        ).Check();
    });
...
#endif //COCOA_GALLIUM_BINDINGS_PRESENT_PROMISIFY_H
