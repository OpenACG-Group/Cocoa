    ...
    , idle_{}
{
    ...
    infra::InstallOnGlobalContext(isolate_, this->context());
...
v8::MaybeLocal<v8::Module> Runtime::getAndCacheSyntheticModule(const ModuleImportURL::SharedPtr& url)
{
    ...
    v8::Local<v8::Module> module = createSyntheticModule(isolate_, binding);
                                             ...
                                             bool isSysInvoke)
{
    ...
    v8::ScriptOrigin scriptOrigin(binder::to_v8(isolate_, resolved->toString()),
                                  0,
                                  0,
                                  false,
                                  -1,
    ...
    if (instantiate.IsNothing())
    {
        if (caught.HasCaught())
        {
            auto what = binder::from_v8<std::string>(
                    isolate_, caught.Exception()->ToString(context()).ToLocalChecked());
