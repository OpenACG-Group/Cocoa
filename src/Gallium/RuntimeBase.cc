                                  ...
                                  const std::string& referrer_url)
{
    ...
    auto referrer_module_cache_itr = std::find_if(
            module_cache.begin(), module_cache.end(),
            [&referrer_url](RuntimeBase::ModuleCacheMap::value_type& pair) {
                            ...
                            g_maybe_unused v8::Local<v8::FixedArray> import_assertions)
{
    ...
    {
        auto message = v8::String::NewFromUtf8Literal(isolate, "Resource name of referrer is undefined");
    ...
    // In that case, we resolve the specifier URL without a relative path.
    std::shared_ptr<ModuleImportURL> referrer_url_info = search_referrer_url_info_in_cache(
            isolate,
            ffi::Cast<std::string>::FromChecked(isolate, resource_name));

    auto specifier_url = ffi::Cast<std::string>::FromChecked(isolate, specifier);
    v8::Local<v8::Module> module;

    constexpr int FLAG = RuntimeBase::kFromImport_ScriptSourceFlag;
    v8::TryCatch catch_block(isolate);

    // TODO(sora): handle the returned Promise object
    if (runtime_base->EvaluateModule(specifier_url, &module, referrer_url_info, FLAG).IsEmpty())
    {
        CHECK(catch_block.HasCaught());
        resolver->Reject(context, catch_block.Exception()).Check();
        return scope.EscapeMaybe(promise);
    }
...
void import_meta_resolve_impl(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    auto url = ffi::Cast<std::string>::FromChecked(isolate, info[0]);
    ...
    info.GetReturnValue().Set(
            ffi::Cast<std::string>::ToChecked(isolate, resolved->ToString()));
}
                                ...
                                v8::Local<v8::Object> meta)
{
    ...
    RuntimeBase::ModuleCacheMap& cache_map = runtime->GetModuleCache();
    ...
    auto url_str = ffi::Cast<std::string>::ToChecked(isolate, import_url->ToString());
...
void RuntimeBase::Initialize()
{
    ...
    ffi_type_context_ = ffi::TypeContext::Make(isolate_);
    bindings::EventEmitterBase::RegisterClass(isolate_);
...
void RuntimeBase::Dispose()
{
    ...
    for (auto& module : module_cache_)
    {
        ...
        module.second.native_module = nullptr;
    }

    {
        ...
        v8::HandleScope handle_scope(isolate_);
        v8::Context::Scope context_scope(GetContext());
        ffi_type_context_->Dispose();
    }

    external_callbacks_.CallGroup(ExternalCallbackType::kAfterDisposeCleanup);
...
v8::Local<v8::Module>
RuntimeBase::ImportNativeModule(const ModuleImportURL::SharedPtr& url)
{
    v8::EscapableHandleScope scope(isolate_);
    CHECK(url->GetProtocol() == ModuleImportURL::Protocol::kNative);
    ...
    ffi::NativeModule *native_module = url->GetNativeModule();

    // Load the dependencies before instantiation
    for (const std::string& native_dep : native_module->GetDeps())
    {
        auto dep_url = ModuleImportURL::Resolve(isolate_, nullptr, "native://" + native_dep,
                                                ModuleImportURL::ResolvedAs::kSysImport);
        CHECK(dep_url);
        ImportNativeModule(dep_url);
    }

    // Instantiate the native module (all the exports are registered here).
    v8::Local<v8::Module> module = native_module->Instantiate(isolate_);
    module_cache_[url] = ESModuleCache(isolate_, module, native_module);
                           ...
                           int script_source_flags)
{
    ...
    v8::Context::Scope context_scope(this->GetContext());
    ...
    ModuleImportURL::SharedPtr resolved = ModuleImportURL::Resolve(
            isolate_, referer.get(), url, resolve_strategy);
    if (!resolved)
    {
        QLOG(LOG_ERROR, "({}) Failed to resolve the module path `{}`", runtime_id_, url);
        isolate_->ThrowError("Failed to resolve the module path");
        return {};
    }
    ...
    for (auto& cache : module_cache_)
    {
        if (*cache.first == *resolved)
            return handle_scope.Escape(cache.second.module.Get(isolate_));
    }

    // Synthetic modules don't need to be compiled
    if (resolved->GetProtocol() == ModuleImportURL::Protocol::kNative)
    {
        v8::Local<v8::Module> module = ImportNativeModule(resolved);
        return handle_scope.Escape(module);
    }

    v8::ScriptOrigin script_origin(
        ffi::Cast<std::string>::ToChecked(isolate_, resolved->ToString()),
        0,
        0,
        false,
        -1,
    ...
    v8::MaybeLocal<v8::String> source_text = ffi::Cast<std::string>::To(
            isolate_, *resolved->LoadResourceText());
    if (source_text.IsEmpty())
    {
        QLOG(LOG_ERROR, "({}) Failed to load the source text from `{}`", runtime_id_, url);
        isolate_->ThrowError("Failed to load the source text");
        return {};
    }

    v8::ScriptCompiler::Source source(source_text.ToLocalChecked(), script_origin);
    v8::Local<v8::Module> module;
                            ...
                            v8::Local<v8::Module> referer)
{
    ...
    for (auto& cache : runtime_base->GetModuleCache())
    {
        if (cache.second.module != referer)
            continue;
        auto url = ffi::Cast<std::string>::FromChecked(isolate, specifier);
...
{
    v8::Local<v8::Module> module;
    if (!CompileModule(referer, url, script_source_flags).ToLocal(&module))
        return {};
    ...
    {
        if (!module->InstantiateModule(context, instantiate_module_callback).FromMaybe(false))
            return {};
    }
...
RuntimeBase::ExecuteScript(const char *script_name, const char *source_str)
{
    ...
    v8::ScriptOrigin origin(
        ffi::Cast<std::string>::ToChecked(isolate_, script_name),
        0,
        0,
        false,
        -1,
    ...
    v8::ScriptCompiler::Source source(
            ffi::Cast<const char*>::ToChecked(isolate_, source_str), origin);
...
v8::MaybeLocal<v8::Value> RuntimeBase::ExecuteScript(const std::string& url, int script_src_flags)
{
    v8::EscapableHandleScope handle_scope(isolate_);
    v8::Local<v8::Context> context = GetContext();
    ...
    ModuleImportURL::ResolvedAs resolve_strategy;
    if (script_src_flags & kFromImport_ScriptSourceFlag)
    {
        resolve_strategy = script_src_flags & kSysInvoke_ScriptSourceFlag
                           ? ModuleImportURL::ResolvedAs::kSysImport
                           : ModuleImportURL::ResolvedAs::kUserImport;
    }
    else
    ...
    auto resolved = ModuleImportURL::Resolve(isolate_, nullptr, url, resolve_strategy);
    if (!resolved)
    {
        isolate_->ThrowError("failed to resolve the script URL");
        return {};
    }

    v8::Local<v8::String> source;
    if (auto text_resource = resolved->LoadResourceText())
        source = v8::String::NewFromUtf8(isolate_, (*text_resource).c_str()).ToLocalChecked();
    else
    {
        isolate_->ThrowError("failed to load script text resource");
        return {};
    }
