...
## 2. Vector Rendering Intermediate Representation (VRIR)
cocanvas 序列化生成的二进制格式称为 __矢量绘图中间表示（VRIR）__。

VRIR 的基本单位是 __命令（Command）__，命令则可以进一步细分为
...
VRIR 在被解释器运行时，始终存在一块随时可以被取用的内存空间，这块空间在进程虚拟地址空间中不一定连续，
因而分配在该内存块内的对象需要使用 __键（Key）__ 来访问，键常常是一个 32 位无符号整数。
这块内存空间便是 VRIR VM 模型的 __堆（Heap）__。VRIR 提供了一系列指令来在堆上创建、拷贝和销毁对象，
...
该常量就有意义了。这类似很多编程语言中提供的枚举类型，但实际上 VRIR 中的枚举是一种「枚举映射」，要完全理解这一点，
...
相比起字符串表示，VRIR 的二进制表示非常朴素，为了节省空间，它舍弃了非常多的细节。

VRIR 二进制表示即 VRIR 字节码，一律采用小端字节序。如下是编码规则：
